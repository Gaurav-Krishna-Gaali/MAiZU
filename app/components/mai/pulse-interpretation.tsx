...
interface PulseInterpretationProps {
  analysisData?: any
}

export function PulseInterpretation({ analysisData }: PulseInterpretationProps) {
  // if API analysis available, show per-position summary instead of chart
  const usingApi = !!analysisData
  ...
  // if we have API data, convert organ_scores into a chart-friendly array;
  // otherwise fall back to the random week values
  let data = generatePulseData(activePosition, activeHand)
  if (usingApi && analysisData.organ_scores) {
    data = Object.entries(analysisData.organ_scores).map(([org, val]) => ({
      day: org,
      value: val as number,
    }))
  }
  ...
  return (
    <div className="rounded-2xl bg-[#ffffff] p-5 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
      ...
      {usingApi && perPos ? (
        <div className="mt-4 text-sm text-[#111827]">
          <h4 className="font-medium">Position analysis</h4>
          <ul className="list-disc list-inside">
            {Object.entries(perPos).map(([pos, info]: any) => (
              <li key={pos}>
                <strong>{pos.replace(/_/g, ' ')}</strong>: {info.dominant_quality} ({info.category})
              </li>
            ))}
          </ul>
        </div>
      ...
      {/* Hand toggle + position selector hidden when using API data */}
      {!usingApi && (
        <>
